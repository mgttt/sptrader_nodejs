...
var user_id='DEMO201705156A';
var host='demo.spsystem.info';
var host_id=80;
var port=8080;
var password='abcd1234';
var license='591E968A664D1';
var app_id="SPDEMO";
//现在是hardcode，应该要从命令行获取}
...
function SubscribeTicker(rst_prev){
	var inst_code='HSI';//TMP
	return Q(QSP).invoke('remote','SPAPI_SubscribeTicker',{user_id,prod_code:'HSIN7',mode:1})
		.then(rst=>{
			if(rst&&(rst.rc==0||rst.rc==1)){
				if(rst.rc==0) return Q.delay(6666).then(()=>{return QSP});//let it complete
				return QSP;
...
TryLogin(1)
	...
	//.then(GetProducts)
	//.then(rst=>{
	//	logger.log('GetProducts.rst=',rst);
	//	return rst;
	//})
	.then(SubscribeTicker)
	.then(rst=>{
		logger.log(`如果注册成功，server端就应该会有类似
2017-06-29 22:39:31.614 TODO on( TickerUpdate ) { ticker: 
{ DealSrc: 1,
DecInPrice: 0,
Price: 25712,
ProdCode: 'HSIN7',
Qty: 2,
TickerTime: 1498747017 } }
这样的返回的了，至于怎么用，自己解决啦!
			`);
		logger.log('SubscribeTicker.rst=',rst);
		return rst;
	})
	.catch(err=>{
